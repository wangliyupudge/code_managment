...
, sum(case when (promote_fudu != 0 or promote_fudu is not null) then promote_fudu else 0 end) as sum_credit_promote_youtie
, sum(promote_fudu) as sum_credit_promote_all
, activation_type
, inloan_type
, sum(baitiao_credit) as sum_baitiao_credit
, sum(calcu_credit) as sum_calcu_credit
, case when promote_fudu != 0 and promote_fudu is not null then '提额' 
  else '未提额' end as is_promote
...
from
    (select
        user_id,
        case when (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) <= 200 and (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) != 0 then 200+baitiao_credit
      when baitiao_credit*gue_xishu*td_xishu*inloan_xishu >= 3000 and 3000+baitiao_credit < 10000 then 3000+baitiao_credit
      when baitiao_credit*gue_xishu*td_xishu*inloan_xishu + baitiao_credit >= 10000 then 10000
      when (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) = 0 then baitiao_credit
      else baitiao_credit
      end as calcu_credit,
        case when (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) <= 200 and (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) != 0 then 200
      when baitiao_credit*gue_xishu*td_xishu*inloan_xishu >= 3000 and 3000+baitiao_credit < 10000 then 3000
      when baitiao_credit*gue_xishu*td_xishu*inloan_xishu + baitiao_credit >= 10000 then 10000-baitiao_credit
      when (baitiao_credit*gue_xishu*td_xishu*inloan_xishu) = 0 then 0
...
, activation_type
    ...
    when zm_score >= 550 and zm_score <= 599 then '550~600'
                ...
                where plat_type = 2
                    and fina_order_cnt != 0
                    and huabei_credit is null
                    and
                    dt = '2018-11-14'
)
risk
...
kexiadan_type
      ...
      , case when length(result_param) < 700 then        ---小于700的情况
       ...
       then cast(json_extract(result_param,'$.vars[4].value') as varchar )
                ...
                WHERE
 last_6m_avg_asset_total_value is null)
as das
...
credit
                ...
                (SELECT
                    user_id,
                    case when date_def >= 0 and date_def <= 30 then '0~30'
           when date_def >= 31 and date_def <= 90 then '31~90'
           when date_def >= 91 and date_def <= 181 then '90~181'
           when date_def >= 181 then '181+' end as activation_type
                from(
select user_id,
                        min(date_diff('day',dat,date('2018-11-15'))) AS date_def
                    FROM
                        (
      select
                            user_id,
                            date(dt) as dat
                        from dw.dwd_sensors_log_d
...
case when date_def >= 0 and date_def <= 30 then '0~30'
           when date_def >= 31 and date_def <= 90 then '31~90'
           when date_def >= 91 and date_def <= 181 then '90~181'
           when date_def >= 181 then '181+' end
)act
                on act.user_id = risk.user_id
